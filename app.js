...
function renderFiles() {
  ...
  document.querySelector('#share-groups').innerHTML = groups.map(share => {
    ...
    return `<article class="share-group ${share.available ? '' : 'unavailable'} ${collapsed ? 'collapsed' : ''}">
      <header class="group-header"><div class="group-name"><span class="folder-icon">▰</span><div><div class="group-title">${escapeHTML(share.name)}</div><div class="group-path">${escapeHTML(share.networkPath)}</div></div></div><div><span class="file-meta">${share.available ? `${share.files.length} 个一级项目` : '服务器暂不可访问'}</span><button class="group-toggle" type="button" data-toggle-share="${escapeHTML(share.id)}" aria-expanded="${!collapsed}"><span class="chevron">⌄</span>${collapsed ? '展开' : '收起'}</button></div></header>
